...
* Make sure that the {MiniZinc}[http://www.minizinc.org/] binary <code>mzn-g12fd</code> is present in the path:
...
The app uses the {rake-deploy-tasks}[https://github.com/gunpowderlabs/buildpack-ruby-rake-deploy-tasks] buildpack to automatically run pending migrations on deploy, as well as the {vendorbinaries}[https://github.com/peterkeen/heroku-buildpack-vendorbinaries] buildpack to pull in MiniZinc for generating recommendations.
