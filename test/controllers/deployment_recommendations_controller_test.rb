...
class DeploymentRecommendationsControllerTest < ActionController::TestCase
  test 'listing recommendations for an ingredient' do
  ...
  test 'destroy all recommendations for an ingredient' do
    rails_app = create(:rails_app, user: @user)
    recommendation = create(:deployment_recommendation, ingredient: rails_app, user: @user)

    delete :destroy_all, { ingredient_id: rails_app.id }
    assert_response :success
    assert_nil DeploymentRecommendation.find_by_id(recommendation.id)
  end
